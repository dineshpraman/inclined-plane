 ...
 * Used for force arrows 
 * Adapted from  Forces and Motion: Basics / ReadoutArrow.js 
 * @modified Dinesh
 */
define( function( require ) {
  ...
  var forceReadoutPattern = require( 'string!EXAMPLE_SIM/forceReadout.pattern' );
   ...
   * @param maxForce{Number} the maximum value of Appliedforce
   ...
   * @constructor
   */
  function ReadoutArrow( label, maxForce, location, valueProperty, showValuesProperty, options ) {
    var readoutArrow = this;

    //Store fields
    this.options = _.extend( {labelPosition: 'top', arrowScale: 10}, options );
    this.showValuesProperty = showValuesProperty;
    this.tailX = location.x;
    this.tailY = location.y;
    ...
    this.arrowNode = new Path( null, _.extend( {fill: 'red', stroke: '#000000', lineWidth: 1}, options ) );
    var fontOptions = {font: new PhetFont( { size: 14, weight: 'bold' } )};
    ...
    valueProperty.link( function( value ) {
      ...
      readoutArrow.valueNode.text = StringUtils.format( forceReadoutPattern, value.toFixed( 2 ) );
      readoutArrow.update();
    } );
  ...
  return inherit( Node, ReadoutArrow, {
	...
	this.arrowNode.fill= (this.value < this.maxForce ) ? 'red':'green';  // change the arrow green if the appliedForce reaches the maxForce
	
//	this.arrowNode.fill = 'red';
        ...
        if ( this.options.labelPosition === 'side' ) {
          ...
          this.labelNode.bottom = this.arrowNode.centerY;
