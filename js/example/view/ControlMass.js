 ...
 * adapted from Gravity Force Lab / ControlMass.js
 * @modified by Dinesh
 * 
 */
...
// function to create the track ---------[]--------
  ...
  function Track( massProperty, massRange ) {
    ...
    Rectangle.call( this, 0, 0, TRACK_SIZE.width, TRACK_SIZE.height, { cursor: 'pointer', fill: "black" } ); //create the track

    var thisNode = this,
      positionToConcentration = new LinearFunction( 0, TRACK_SIZE.width, massRange.min, massRange.max, true ), //create a linear function to correspond to length of the track and mass range 

      handleEvent = function( event ) {
        massProperty.set( Math.round( positionToConcentration( thisNode.globalToLocalPoint( event.pointer.point ).x ) ) );
      }; //change the mass according to the position
...
//The consstructor function
  function ControlMass( simTitle, massProperty, massRange, options ) {
    ...
    var title = new Text( simTitle, { font:  { size: 18 , weight: 'bold' }, pickable: false } );
...
/* 
   this.addChild( new Panel( content, {
     fill: options.fill, xMargin: options.xMargin, yMargin: options.yMargin, scale: options.scale, resize: false, lineWidth: options.lineWidth, } ) );
*/

    massProperty.link( function updateMass( value ) {
      valueLabel.text = StringUtils.format( pattern_0value_1units, value, units);
